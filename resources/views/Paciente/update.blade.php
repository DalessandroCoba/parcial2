...
<body>
  ...
  <div class="card-body">
    <form method="POST" action="{{ route('paciente.update', $pacientes->id_Paciente) }}">
        @csrf
        @method("PUT")
        <div class="row">
        <div class="col mb-3">
          <label for="documento" class="form-label">Cedula</label>
          <input type="number" class="form-control" id="documento" aria-describedby="documento" name="documento" value="{{ $pacientes->id_Paciente }}">
        ...
        <div class="col mb-3">
          ...
          <input type="text" required class="form-control" id="nombres"  aria-describedby="nombres"
            name="nombres" placeholder="nombres" value="{{ $pacientes->nom_Paciente }}">
        </div>
    </div>
    ...
    <div class="row">
        <div class="col mb-3">
            ...
            <input type="number" required class="form-control" id="edad"  aria-describedby="edad"
              name="edad" placeholder="edad" value="{{ $pacientes->age_Paciente }}">
        </div>

        <div class="col mb-3">
            <label for="telefono" class="form-label">Telefono</label>
            <input type="number" required class="form-control" id="telefono"  aria-describedby="telefono"
              name="telefono" placeholder="telefono" value="{{ $pacientes->tel_Paciente  }}">
        </div>
    </div>
    ...
    <div class="row">
        <div class="col mb-3">
            ...
            <input type="email" required class="form-control" id="correo"  aria-describedby="correo"
              name="correo" placeholder="correo" value="{{ $pacientes->email_Paciente }}">
        </div>

        <div class="col mb-3">
            <label for="direccion" class="form-label">Direccion</label>
            <input type="text" required class="form-control" id="direccion"  aria-describedby="direccion"
              name="direccion" placeholder="direccion" value="{{ $pacientes->dir_Paciente }}">
        </div>
    </div>
    ...
    <select class="form-select" id="estado" name="estado" required>
        <option selected disabled value="{{ $pacientes->estado }}">Choose one...</option>
